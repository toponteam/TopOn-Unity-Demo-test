...
//    }
    ...
    public static void addNativeAdView2Activity(final Activity pActivity, final ViewInfo pViewInfo, final View adView) {

        if (pActivity == null || adView == null) {
        ...
        pActivity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    ViewGroup _viewGroup = (ViewGroup) adView.getParent();
                    if (_viewGroup != null) {
                        _viewGroup.removeView(adView);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                ...
                if(pViewInfo.rootView != null){
                    ...
                    if(!TextUtils.isEmpty(pViewInfo.rootView.bgcolor)){
                        adView.setBackgroundColor(Color.parseColor(pViewInfo.rootView.bgcolor));
                    }

                    MsgTools.pirntMsg("Add native view to content start....");
                    pActivity.addContentView(adView, layoutParams);
